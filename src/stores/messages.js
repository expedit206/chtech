...
export const useMessageStore = defineStore("message", () => {
  ...
  const fetchConversations = async (
    ...
    if (
      ...
      silent &&
      !loadMore &&
      lastFetchedConversations.value &&
      Date.now() - lastFetchedConversations.value < CONV_CACHE_TIMEOUT
    ) {
      return;
    }
    ...
    try {
      ...
      if (loadMore) {
        const existingKeys = new Set(
          conversations.value.map(
            (c) => c.user_id + "_" + (c.product_id || ""),
          ),
        );
        const newConvs = response.data.conversations.filter(
          (c) => !existingKeys.has(c.user_id + "_" + (c.product_id || "")),
        );
        ...
        if (conversations.value.length === 0 || force || !silent) {
          ...
          fetched.forEach((conv) => {
            const isCurrentlyOpen =
              ...
              String(selectedConversation.value.user_id) ===
                String(conv.user_id) &&
              String(selectedConversation.value.product_id || "") ===
            ...
            const idx = conversations.value.findIndex(
              (c) =>
                ...
                String(c.product_id || "") === String(conv.product_id || ""),
            );
            if (idx !== -1) {
      ...
      if (!silent && !loadMore)
        flash.error("Erreur lors du chargement des conversations");
    } finally {
  ...
  const fetchMessages = async (
    ...
    if (resetOffset) {
      ...
      const convMatch = conversations.value.find(
        (c) =>
          String(c.user_id) === String(receiverId) &&
          String(c.product_id || "") === String(productId || ""),
      );
    ...
    if (
      !resetOffset &&
      lastFetchedMessages.value[cacheKey] &&
      now - lastFetchedMessages.value[cacheKey] < MSG_CACHE_TIMEOUT
    ) {
      return;
    }
      ...
      if (response.data.user) {
        ...
        if (convMatch && convMatch.name) {
          ...
          const clientName =
            ...
            authStore.user?.email === "aaa@aaa.com"
              ? user.nom
              : authStore.user?.nom;
        ...
        selectedConversation.value = {
          ...
          product_name: productData
            ? productData.nom
            : convMatch
              ? convMatch.product_name
              : null,
          product_slug: productData
            ? productData.slug
            : convMatch
              ? convMatch.product_slug
              : null,
          product_owner_id: productData
            ? productData.user_id ||
              (productData.user ? productData.user.id : null)
            : convMatch
              ? convMatch.product_owner_id
              : null,
          product_image:
            productData && productData.photos
              ? productData.photos[0]
              : convMatch
                ? convMatch.product_image
                : null,
          product_min_price: productData
            ? productData.prix_minimum
            : convMatch
              ? convMatch.product_min_price
              : null,
  ...
  const updateConversationLastMessage = (message) => {
    const cid =
      String(message.sender_id) === String(authStore.user?.id)
        ? message.receiver_id
        : message.sender_id;
    const pid = message.product_id;
    const convIndex = conversations.value.findIndex(
      (c) =>
        String(c.user_id) === String(cid) &&
        String(c.product_id || "") === String(pid || ""),
    );
    ...
    if (convIndex !== -1) {
      ...
      selectedConversation.value &&
      String(selectedConversation.value.user_id) === String(cid) &&
      String(selectedConversation.value.product_id || "") === String(pid || "")
    ) {
      ...
      const newConv = {
        ...
        user_name:
          selectedConversation.value.user_name ||
          (authStore.user?.role === "admin"
  ...
  const editMessage = async (messageId, content) => {
    try {
      const response = await apiClient.put(`/chat/message/${messageId}`, {
        content,
      });
  ...
  const sendPendingCartIfAny = async () => {
    ...
    if (!cartMeta.value) {
      ...
      if (storedMeta) {
        try {
          cartMeta.value = JSON.parse(storedMeta);
        } catch {
          /* ignore */
        }
      }
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.ondataavailable = (e) => {
        if (e.data.size > 0) recordingState.value.audioChunks.push(e.data);
      };
  ...
  const stopRecordingTimer = () => {
    if (recordingState.value.timer) clearInterval(recordingState.value.timer);
  };
