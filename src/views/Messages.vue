    ...
    >
      ...
      >
        ...
        <div
          ...
          class="flex-1 flex justify-between items-center p-2 md:p-4 min-w-0"
        >
          <div class="flex items-center gap-3 min-w-0 flex-1">
            ...
            <router-link
              ...
              :to="{
                name: 'DetailProduit',
                params: {
                  slug: messageStore.selectedConversation.product_slug,
                },
              }"
            ...
            <div
              v-else
              class="w-10 h-10 rounded-full bg-[var(--color-primary)] text-white flex items-center justify-center font-bold shrink-0"
            >
              {{ messageStore.selectedConversation.name?.charAt(0) }}
            </div>
            ...
            <div class="flex flex-col min-w-0 pr-2">
              <component
                :is="
                  messageStore.selectedConversation.product_slug
                    ? 'router-link'
                    : 'span'
                "
                :to="
              ...
              <span
                v-if="
                  authStore.isAdmin &&
                  messageStore.selectedConversation.product_name
                "
                class="text-[10px] font-semibold opacity-60 truncate uppercase tracking-wider"
        ...
        <div
          v-else
          class="flex-1 flex items-center justify-center p-4 opacity-50"
        >
      ...
      <div class="flex-1 min-h-0 relative flex flex-col">
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
              <div
                class="w-8 h-8 rounded-lg bg-[var(--color-primary)] text-white flex items-center justify-center shadow-sm text-sm"
              >
              ...
              <div>
                <span
                  ...
                  :style="{ color: 'var(--color-text-sub)' }"
                  >Panier à valider</span
                >
                <p
                  class="text-xs font-black"
                  :style="{ color: 'var(--color-text-main)' }"
                >
                  {{ cartOrderItems.length }} article{{
                    cartOrderItems.length > 1 ? "s" : ""
                  }}
            ...
            <div class="flex items-center gap-3">
              ...
              >
                <span
                  ...
                  :class="orderStatusDotClass(cartOrderStatus)"
                ></span>
              ...
              >
                <i
                  ...
                  :class="
                    isCartPanelExpanded ? 'fa-chevron-down' : 'fa-chevron-up'
                  "
                ></i>
              </button>
            </div>
          ...
          <div
            v-if="cartDeliveryAddress"
            class="flex items-center gap-2 mb-3 px-2 py-1.5 rounded-lg"
            :style="{
          ...
          >
            <i
              class="fas fa-map-marker-alt text-[10px]"
              :style="{ color: 'var(--color-primary)' }"
            ></i>
            <span
              class="text-[10px] font-bold truncate"
              :style="{ color: 'var(--color-primary)' }"
              >{{ cartDeliveryAddress }}</span
            >
          </div>

          <!-- Items list (Collapsible) -->
          <div
            v-show="isCartPanelExpanded"
            class="space-y-2 mb-4 max-h-[180px] overflow-y-auto pr-2 custom-scrollbar"
          >
            <div
              ...
              "
              :style="{
                backgroundColor: item.selected ? '' : 'var(--color-bg)',
              }"
            >
              <!-- Checkbox -->
              <input
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mt-0.5">
                  <!-- Quantity -->
                  <div
                    class="flex items-center gap-1 border rounded-lg overflow-hidden"
                    :style="{ borderColor: 'var(--color-border)' }"
                  >
                    <button
                      @click="item.quantity = Math.max(1, item.quantity - 1)"
                      class="w-5 h-5 text-[10px] flex items-center justify-center hover:bg-black/10 font-bold"
                    >
                      -
                    </button>
                    <span class="text-[10px] font-black w-5 text-center">{{
                      item.quantity
                    }}</span>
                    <button
                      @click="item.quantity++"
                  ...
                  <div class="relative flex items-center">
                    ...
                    />
                    <span
                      class="absolute right-2 text-[9px] font-bold opacity-40"
                      :style="{ color: 'var(--color-text-sub)' }"
                      >CFA</span
                    >
                  </div>
          ...
          <div v-if="!cartOrderStatus" class="flex items-center gap-3">
            ...
            >
              ...
              <span
                class="font-black"
                :style="{ color: 'var(--color-primary)' }"
                >{{ formatMoney(cartSelectedTotal) }}</span
              >
            </div>
            <button
            ...
            >
              ...
              <span>{{
                isCreatingOrder
                  ? "Validation..."
                  : `VALIDER (${cartSelectedItems.length} art.)`
              }}</span>
            </button>
          </div>
        ...
        >
          <div class="flex items-center gap-2">
            <div
              class="w-8 h-8 rounded-lg bg-[var(--color-primary)] text-white flex items-center justify-center shadow-sm"
            >
            ...
            <div class="flex flex-col">
              <span
                class="text-[10px] font-bold uppercase tracking-wider opacity-60"
                :style="{ color: 'var(--color-text-sub)' }"
                >Gestion de commande</span
              >
              <span
                class="text-xs font-black truncate max-w-[150px] md:max-w-none"
                :style="{ color: 'var(--color-text-main)' }"
                >{{ messageStore.selectedConversation.product_name }}</span
              >
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 ml-auto">
            <template v-if="!messageStore.selectedConversation.order_status">
              <div
                class="flex flex-col items-end gap-1 mr-2 px-3 border-r border-white/10"
              >
                <span
                  class="text-[9px] font-black opacity-50 uppercase tracking-tighter"
                  :style="{ color: 'var(--color-text-sub)' }"
                  >Prix final négocié</span
                >
                <div class="relative flex items-center">
                  <input
                    type="number"
                  ...
                  />
                  <span
                    class="absolute right-2 text-[10px] font-bold opacity-40"
                    >CFA</span
                  >
                </div>
              </div>
...
const orderStatusBadgeClass = (s) =>
  ORDER_STATUS_BADGE[s] || "bg-gray-500/10 text-gray-600 border-gray-500/20";
...
const cartSelectedItems = computed(() =>
  cartOrderItems.value.filter((i) => i.selected),
);
...
const createCartOrder = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      toast.success(
        `Commande validée — ${cartSelectedItems.value.length} article(s).`,
      );
...
const updateOrderStatus = async (status) => {
  ...
  const orderId =
    cartOrderId.value || messageStore.selectedConversation?.order_id;
  if (!orderId) {
...
onMounted(async () => {
  ...
  if (route.params.receiverId) {
    await messageStore
      .fetchMessages(route.params.receiverId, true, route.query.productId)
      .catch(console.error);
...
watch(
  () => authStore.user?.id,
  (userId) => {
    if (userId) messageStore.subscribeToChannel();
  },
);
</script>
