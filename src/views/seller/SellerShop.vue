<template>
  <div class="p-4 md:p-8 space-y-8 animate-fade-in">
    ...
    >
      <!-- Cover banner -->
      <div
        class="h-36 md:h-48 relative overflow-hidden bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 group"
      >
        <img
          v-if="coverUrl"
          :src="coverUrl"
        ...
        <label v-else class="absolute bottom-3 right-3 cursor-pointer">
          ...
          />
          <span
            class="flex items-center gap-2 px-3 py-1.5 bg-black/50 text-white rounded-xl text-xs font-bold backdrop-blur-sm hover:bg-black/70 transition-all hover:scale-105 active:scale-95"
          >
            <Camera :size="14" />
            {{
              auth.user?.cover ? "Changer la bannière" : "Ajouter une bannière"
            }}
          </span>
        </label>
      </div>
      ...
      <div class="flex flex-col md:flex-row gap-4 p-6 pt-0">
        <div class="relative -mt-10 shrink-0 group">
          <div class="relative w-24 h-24 md:w-32 md:h-32">
            <img
              :src="avatarUrl"
              class="w-full h-full rounded-full border-4 object-cover shadow-lg transition-all"
            ...
            <div
              ...
              class="absolute inset-0 bg-black/50 rounded-full flex items-center justify-center text-white backdrop-blur-[2px]"
            >
            ...
            <label
              v-else
              class="absolute -bottom-1 -right-1 cursor-pointer w-8 h-8 rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-110 active:scale-95"
        ...
        <div class="flex-1 pt-2 md:pt-4">
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p
                class="text-sm mt-0.5 opacity-60"
                :style="{ color: 'var(--color-text-main)' }"
              >
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div
        class="rounded-2xl border p-6 space-y-4 shadow-sm"
        :style="{
          backgroundColor: 'var(--color-surface)',
          borderColor: 'var(--color-border)',
        }"
      >
        <h2
          class="font-black text-base flex items-center gap-2"
          :style="{ color: 'var(--color-text-main)' }"
        >
          <span
      ...
      <div class="space-y-6">
        ...
        >
          ...
          >
            <span
              class="w-8 h-8 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-600"
            >
              <Globe :size="18" />
            </span>
            Réseaux & Contact
          ...
          <div class="space-y-4 pt-2">
            ...
            <div>
              ...
              <div class="relative">
                <span
                  class="absolute left-4 top-1/2 -translate-y-1/2 opacity-40 text-sm font-bold"
                  >@</span
                >
                <input
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-3">
            <router-link
              :to="{ name: 'my-products', query: { add: 'true' } }"
            ...
            <router-link
              ...
              :style="{ color: 'var(--color-text-main)' }"
            >
              <ShoppingCart :size="20" class="text-orange-500" /> Ventes
              (Commandes)
            </router-link>
          </div>
        </div>
...
useSeo({
  ...
  description:
    "Gérez l'apparence et les informations de votre boutique vendeur sur SASAYEE.",
});
...
const avatarUrl = computed(() => {
  ...
  if (!photo)
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(auth.user?.nom || "V")}&background=6366f1&color=fff&size=256`;
...
const onCoverSelect = async (e) => {
  ...
  try {
    ...
    console.error(err);
    flash.error(
      err.response?.data?.message || "Erreur lors de l'envoi de la bannière",
    );
  } finally {
