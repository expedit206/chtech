<template>
  <div class="p-4 md:p-8 animate-fade-in">
    <!-- Page Header -->
    <div class="mb-6">
      <h1 class="text-2xl font-black" :style="{ color: 'var(--color-text-main)' }">Utilisateurs</h1>
      <p class="text-sm mt-1" :style="{ color: 'var(--color-text-sub)' }">{{ totalUsers }} utilisateurs enregistrés</p>
    </div>
    ...
    <div class="rounded-2xl border overflow-hidden"
      ...
      <!-- Empty -->
      <div v-else-if="users.length === 0" class="flex flex-col items-center py-20 gap-3">
        <Users :size="48" class="opacity-10" :style="{ color: 'var(--color-text-main)' }" />
        <p class="font-bold opacity-40" :style="{ color: 'var(--color-text-sub)' }">Aucun utilisateur trouvé</p>
      </div>
            ...
            <tr v-for="user in users" :key="user.id"
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <img :src="avatarUrl(user)" class="w-9 h-9 rounded-full object-cover border"
                    :style="{ borderColor: 'var(--color-border)' }" />
                  <div class="min-w-0">
                    <p class="font-bold text-sm truncate max-w-[140px]">{{ user.nom }}</p>
                    <p v-if="user.premium" class="text-[10px] font-black text-amber-500">⭐ Premium</p>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 hidden sm:table-cell">
                <p class="text-sm opacity-70 truncate max-w-[160px]">{{ user.email || user.telephone }}</p>
              </td>
              <td class="px-6 py-4">
                <span class="text-[11px] font-black px-2.5 py-1 rounded-full uppercase"
                  :class="{
              ...
              <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <select @change="changeRole(user.id, $event.target.value)" :value="user.role"
      ...
      <div v-if="lastPage > 1" class="flex items-center justify-between px-6 py-4 border-t"
        :style="{ borderColor: 'var(--color-border)' }">
        <span class="text-xs opacity-50" :style="{ color: 'var(--color-text-sub)' }">
        ...
        <div class="flex gap-2">
          ...
          <button @click="goPage(currentPage + 1)" :disabled="currentPage >= lastPage"
            class="px-3 py-1.5 rounded-lg text-xs font-bold border disabled:opacity-30 transition"
            :style="{ borderColor: 'var(--color-border)', color: 'var(--color-text-main)' }">
            Suivant →
          </button>
...
const goPage = (p) => { if (p >= 1 && p <= lastPage.value) fetchUsers(p); };
