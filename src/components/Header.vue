<template>
  <header
    class="fixed top-0 z-50 w-full shadow-sm border-b transition-all duration-300 header-main"
    :class="{
  ...
  >
      ...
      <div class="flex items-center justify-between px-4 h-16 md:h-18 gap-4">
        <div class="flex items-center gap-3">
          ...
          <button
            ...
            class="md:hidden p-2 -ml-2 rounded-lg hover:bg-black/5"
    ...
    >
      <div
        ...
        class="md:hidden px-4 pb-3 border-b overflow-hidden"
        :style="{
          backgroundColor: 'var(--color-surface)',
          borderColor: 'var(--color-border)',
        }"
      >
        <div
          class="flex items-center gap-3 px-4 py-2 rounded-full cursor-pointer"
          :style="{ backgroundColor: 'var(--color-bg)' }"
          @click="openSearchOverlay"
        >
    ...
    <div
      ...
      class="fixed inset-0 z-[100] flex flex-col animate-in fade-in duration-200"
      :class="{
        'header-offset': auth.isAuthenticated && !isSidebarCollapsed,
        'header-offset-collapsed': auth.isAuthenticated && isSidebarCollapsed,
      }"
    ...
    >
      <!-- Overlay Header -->
      <div
        class="h-16 flex items-center gap-4 px-4 border-b"
        :style="{ borderColor: 'var(--color-border)' }"
      >
        <button
          @click="closeSearchOverlay"
          class="p-2 rounded-full hover:bg-[var(--color-bg)] transition-colors"
          :style="{ color: 'var(--color-text-main)' }"
        >
          <ArrowLeft :size="24" />
        </button>
        ...
        <div class="flex-1 relative">
          <input
            ...
            @keyup.enter="handleFinalSearch"
            class="w-full bg-transparent border-none outline-none text-lg"
            :style="{ color: 'var(--color-text-main)' }"
          />
          <button
        ...
        <button
          @click="handleFinalSearch"
          :disabled="!productStore.searchQuery.trim()"
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        <div v-if="liveResults.length > 0" class="p-4">
          ...
          <div class="space-y-1">
            ...
            >
              <div
                class="w-12 h-12 rounded-xl overflow-hidden border flex-shrink-0"
                :style="{ borderColor: 'var(--color-border)' }"
              >
                <img
                  v-if="item.result_type === 'product'"
                  :src="item.image || '/placeholder.png'"
                  class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                />
                <div
                  v-else
                  class="w-full h-full flex items-center justify-center bg-[var(--color-primary)]/10"
                >
                  <Briefcase
                    :size="20"
        ...
        <div
          v-if="!productStore.searchQuery && searchHistory.length > 0"
          class="p-4"
        >
          <div class="flex items-center justify-between mb-4">
            <h3
        ...
        >
          ...
          <button
            @click="handleFinalSearch"
            class="mt-4 text-sm font-bold text-[var(--color-primary)] hover:underline"
          >
...
const handleAddProductClick = () => {
  if (auth.isVendeur || auth.isAdmin) {
    ...
    alertStore.showAlert({
      ...
      message:
        "Vous devez posséder un compte vendeur pour pouvoir vendre des produits. Souhaitez-vous en créer un maintenant ?",
      type: "info",
