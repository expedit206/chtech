      ...
      >
        ...
        <div class="flex items-center justify-between px-5 py-4 border-b border-[var(--color-border)] shrink-0">
          <div class="flex items-center gap-3">
            ...
            <h2 class="text-lg font-black" :style="{ color: 'var(--color-text-main)' }">
              Mon Panier
              <span class="ml-2 text-sm font-bold opacity-50">({{ cartStore.totalItems }} article{{ cartStore.totalItems > 1 ? 's' : '' }})</span>
            </h2>
          </div>
          <button
            @click="cartStore.isDrawerOpen = false"
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
          <!-- Empty state -->
          <div v-if="cartStore.isEmpty" class="flex flex-col items-center justify-center h-full gap-4 text-center px-8">
            <div class="w-20 h-20 rounded-2xl bg-[var(--color-border)]/20 flex items-center justify-center">
              <ShoppingCart :size="36" class="opacity-30" :style="{ color: 'var(--color-text-sub)' }" />
            </div>
            <p class="font-bold text-lg" :style="{ color: 'var(--color-text-main)' }">Votre panier est vide</p>
            <p class="text-sm opacity-60" :style="{ color: 'var(--color-text-sub)' }">Ajoutez des produits depuis la marketplace</p>
            <router-link
              :to="{ name: 'Home' }"
              @click="cartStore.isDrawerOpen = false"
          ...
          <ul v-else class="p-4 space-y-3">
            ...
            >
              <!-- Image -->
              <div class="w-16 h-16 rounded-xl overflow-hidden border border-[var(--color-border)] shrink-0 bg-[var(--color-surface)]">
                <img
                  v-if="item.image"
                  :src="item.image"
                  :alt="item.name"
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs font-black mt-1" :style="{ color: 'var(--color-primary)' }">
                  {{ formatMoney(item.priceRaw * item.quantity) }}
                </p>
                <p class="text-[10px] opacity-50 mt-0.5" :style="{ color: 'var(--color-text-sub)' }">
                  {{ formatMoney(item.priceRaw) }} × {{ item.quantity }}
                </p>
              </div>
              ...
              <div class="flex flex-col items-end gap-2 shrink-0">
                <button @click="cartStore.removeItem(item.productId)" class="p-1 rounded-lg text-red-400 hover:bg-red-500/10 transition-colors">
                  <Trash2 :size="14" />
                </button>
                <div class="flex items-center gap-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg overflow-hidden">
                  <button
                  ...
                  <span class="text-xs font-black w-6 text-center" :style="{ color: 'var(--color-text-main)' }">
                    {{ item.quantity }}
                  </span>
                  <button
                    @click="cartStore.updateQuantity(item.productId, item.quantity + 1)"
        ...
        <!-- Footer -->
        <div v-if="!cartStore.isEmpty" class="shrink-0 border-t border-[var(--color-border)] p-4 space-y-3">
          <!-- Total -->
          <div class="flex justify-between items-center">
            <span class="text-sm font-bold" :style="{ color: 'var(--color-text-sub)' }">Total estimé</span>
            <span class="text-xl font-black" :style="{ color: 'var(--color-primary)' }">{{ formatMoney(cartStore.totalPrice) }}</span>
          </div>

          <!-- Delivery address (optional) -->
          <div>
            <label class="text-[10px] font-black uppercase tracking-wider opacity-60 mb-1 flex items-center gap-1.5" :style="{ color: 'var(--color-text-sub)' }">
              <MapPin :size="11" />
              Adresse de livraison <span class="opacity-50 normal-case font-medium">(optionnel)</span>
            </label>
            <input
          ...
          <button
            ...
            class="w-full py-3.5 rounded-2xl font-black text-white flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95 hover:opacity-90 disabled:opacity-60"
            :style="{ backgroundColor: 'var(--color-primary)' }"
          >
            <i v-if="isProceedingToChat" class="fas fa-spinner fa-spin" />
            <MessageCircle v-else :size="18" />
            <span>{{ isProceedingToChat ? 'Ouverture du chat...' : 'Passer une commande' }}</span>
          </button>
...
import { ShoppingCart, X, Trash2, Minus, Plus, Package, MessageCircle, MapPin } from 'lucide-vue-next';
...
const proceedToChat = async () => {
  if (!authStore.isAuthenticated) {
    alertStore.showAlert({
      ...
      showCancel: true,
      onConfirm: () => router.push({ name: 'Login', query: { redirect: router.currentRoute.value.fullPath } }),
    });
    return;
  }
  ...
  try {
    ...
    if (!adminId) {
      alertStore.showAlert({ title: 'Erreur', message: 'Service client indisponible.', type: 'error' });
      return;
    }
    ...
    alertStore.showAlert({ title: 'Erreur', message: "Impossible d'ouvrir le chat. Veuillez réessayer.", type: 'error' });
  } finally {
...
.slide-drawer-enter-active, .slide-drawer-leave-active { transition: transform 0.35s cubic-bezier(0.16,1,0.3,1); }
